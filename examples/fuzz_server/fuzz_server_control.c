...
static ControlHandlerResult controlHandlerForBinaryOutput(ControlAction action,
                                                          void *parameter,
                                                          MmsValue *value,
                                                          bool test) {
  ...
  if (parameter == IEDMODEL_GenericIO_GGIO1_SPCSO1) {
    IedServer_updateUTCTimeAttributeValue(
        iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO1_t, timestamp);
    IedServer_updateAttributeValue(
        iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO1_stVal, value);
  }

  else if (parameter == IEDMODEL_GenericIO_GGIO1_SPCSO2) {
    IedServer_updateUTCTimeAttributeValue(
        iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO2_t, timestamp);
    IedServer_updateAttributeValue(
        iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO2_stVal, value);
  }

  else if (parameter == IEDMODEL_GenericIO_GGIO1_SPCSO3) {
    IedServer_updateUTCTimeAttributeValue(
        iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO3_t, timestamp);
    IedServer_updateAttributeValue(
        iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO3_stVal, value);
  }

  else if (parameter == IEDMODEL_GenericIO_GGIO1_SPCSO4) {
    IedServer_updateUTCTimeAttributeValue(
        iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO4_t, timestamp);
    IedServer_updateAttributeValue(
        iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO4_stVal, value);
  } else
...
static MmsDataAccessError writeAccessHandler(DataAttribute *dataAttribute,
  ...
  if ((ctlModelVal == CONTROL_MODEL_STATUS_ONLY) ||
    ...
    IedServer_updateCtlModel(iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO1,
                             ctlModelVal);
...
int main(int argc, char **argv) {
  ...
  /*
   * For SPCSO1 we want the user be able to change the control model by online
   * service - so we install a write access handler to change the control model
   * when the client writes to the "ctlModel" attribute.
   */
  ...
  /* this is optional - performs operative checks */
  IedServer_setPerformCheckHandler(iedServer, IEDMODEL_GenericIO_GGIO1_SPCSO9,
                                   checkHandler,
                                   IEDMODEL_GenericIO_GGIO1_SPCSO9);
